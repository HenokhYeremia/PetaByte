...
#[tauri::command]
pub async fn scan_cache(state: tauri::State<'_, AppState>) -> Result<Vec<petabyte_shared_models::entities::CacheEntry>, String> {
...
pub async fn cache_total_size(state: tauri::State<'_, AppState>) -> Result<u64, String> {
    state.clean_uc.calculate_total_size().map_err(|e| e.to_string())
}
