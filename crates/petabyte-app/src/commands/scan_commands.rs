...
pub async fn start_scan(
    ...
    let scanner =
        petabyte_scanner::Scanner::new(scanner_config).map_err(|e| e.to_string())?;
    ...
    *state.current_cancel.lock().map_err(|e| e.to_string())? = Some(cancel.clone());
    ...
    let scan_fn: petabyte_core_engine::use_cases::ScanFunction = Arc::new(move |handler, ext_cancel| {
    ...
    let result = tokio::task::spawn_blocking(move || {
        scan_uc.execute(&config, scan_fn, &cancel)
    })
    .await
