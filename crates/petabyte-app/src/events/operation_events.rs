...
pub fn emit_move_complete(handle: &tauri::AppHandle, source: &str, destination: &str, file_size: u64) {
    let _ = handle.emit("move:done", serde_json::json!({
        "source": source,
...
pub fn emit_clean_complete(handle: &tauri::AppHandle, bytes_freed: u64, entries_removed: usize) {
    let _ = handle.emit("clean:done", serde_json::json!({
