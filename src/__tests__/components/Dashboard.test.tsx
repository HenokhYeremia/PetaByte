...
describe("StorageOverviewCard", () => {
  it("renders with data", () => {
    render(<StorageOverviewCard data={mockDashboardData.storage} />);
...
describe("HealthScoreCard", () => {
  it("renders with score", () => {
    render(<HealthScoreCard data={mockDashboardData.health} />);
  ...
  it("renders grade A for score >= 90", () => {
    render(<HealthScoreCard data={{ overall_score: 95, grade: "A", status_label: "Excellent" }} />);
...
describe("DuplicateFilesCard", () => {
  it("renders with data", () => {
    render(<DuplicateFilesCard data={mockDashboardData.duplicates} />);
  ...
  it("renders empty state", () => {
    render(<DuplicateFilesCard data={{ group_count: 0, total_wasted_bytes: 0 }} />);
...
describe("CacheCleanerCard", () => {
  it("renders with data", () => {
    render(<CacheCleanerCard data={mockDashboardData.cache} />);
    expect(screen.getByText("Cache Cleaner")).toBeInTheDocument();
    expect(screen.getByText("7.6 GB")).toBeInTheDocument();
  });

  it("renders empty state", () => {
    render(<CacheCleanerCard data={{ total_cache_bytes: 0, safe_to_remove_bytes: 0, category_count: 0 }} />);
...
describe("LargeFilesCard", () => {
  it("renders with data", () => {
    render(<LargeFilesCard data={mockDashboardData.largeFiles} />);
    expect(screen.getByText("Large Files")).toBeInTheDocument();
    expect(screen.getByText("87")).toBeInTheDocument();
  });

  it("renders empty state", () => {
    render(<LargeFilesCard data={{ file_count: 0, total_size_bytes: 0, threshold_bytes: 100_000_000 }} />);
...
describe("RecentScanCard", () => {
  it("renders with completed scan data", () => {
    render(<RecentScanCard data={mockDashboardData.recentScan} />);
...
describe("ScanStatusWidget", () => {
  it("renders idle state", () => {
    render(<ScanStatusWidget data={{ status: "idle", progress_percent: 0, current_file: null }} />);
    expect(screen.getByText("Idle")).toBeInTheDocument();
  });

  it("renders running state with progress", () => {
    render(<ScanStatusWidget data={{ status: "running", progress_percent: 57, current_file: "file.txt" }} />);
  ...
  it("renders completed state", () => {
    render(<ScanStatusWidget data={{ status: "completed", progress_percent: 100, current_file: null }} />);
    expect(screen.getByText("Completed")).toBeInTheDocument();
  });

  it("renders failed state", () => {
    render(<ScanStatusWidget data={{ status: "failed", progress_percent: 0, current_file: null }} />);
